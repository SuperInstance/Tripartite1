...
///
/// # Arguments
/// * `cert_path` - Path to device certificate (PEM format)
/// * `key_path` - Path to device private key (PEM format)
///
/// # Returns
...
pub fn create_tls_config(
    ...
    let cert_vec = certs(&mut cert_reader).map_err(|e| {
        CloudError::certificate(format!("Failed to parse certificate: {}", e))
    })?;
    let certs = cert_vec.into_iter().map(Certificate).collect::<Vec<_>>();

    if certs.is_empty() {
        return Err(CloudError::certificate("No certificates found in file"));
    }

    // Load device private key (try RSA, then EC, then PKCS8)
    let key_file = File::open(key_path).map_err(|e| {
        CloudError::certificate(format!("Failed to open key file: {}", e))
    })?;
    let mut key_reader = BufReader::new(key_file);

    // Try RSA private keys first
    let keys = rsa_private_keys(&mut key_reader).map_err(|e| {
        CloudError::certificate(format!("Failed to parse RSA key: {}", e))
    })?;
    ...
    let key = if !keys.is_empty() {
        ...
        key_reader = BufReader::new(File::open(key_path).map_err(|e| {
            CloudError::certificate(format!("Failed to reopen key file: {}", e))
        })?);
        let keys = ec_private_keys(&mut key_reader).map_err(|e| {
            CloudError::certificate(format!("Failed to parse EC key: {}", e))
        })?;
        ...
        if !keys.is_empty() {
            ...
            let keys = pkcs8_private_keys(&mut key_reader).map_err(|e| {
                CloudError::certificate(format!("Failed to parse PKCS8 key: {}", e))
            })?;

            if !keys.is_empty() {
                keys[0].clone()
            } else {
                return Err(CloudError::certificate("No private key found in file"));
            }
        }
    };
...
/// In production, certificates would be signed by the SuperInstance CA.
///
/// # Arguments
/// * `device_id` - Unique device identifier
///
/// # Returns
/// * `(Certificate, PrivateKey)` pair for the device
pub fn generate_device_certificate(
    device_id: &str,
) -> CloudResult<(Certificate, PrivateKey)> {
    use rcgen::{Certificate as RcgenCert, CertificateParams, DnType, KeyPair, SanType};

    // Generate key pair
    let key_pair = KeyPair::generate(&rcgen::PKCS_ECDSA_P256_SHA256)
        .map_err(|e| CloudError::certificate(format!("Failed to generate key pair: {}", e)))?;
    ...
    // Subject alternative name (DNS name for device)
    params.subject_alt_names = vec![SanType::DnsName(format!("{}.device.superinstance.ai", device_id))];
    ...
    let cert = RcgenCert::from_params(params)
        .map_err(|e| CloudError::certificate(format!("Failed to generate certificate: {}", e)))?;

    // Convert to rustls types
    let cert_der = cert.serialize_der()
        .map_err(|e| CloudError::certificate(format!("Failed to serialize certificate: {}", e)))?;
...
mod tests {
    ...
    fn test_generate_device_certificate() {
        ...
        let (cert, key) = generate_device_certificate(device_id).unwrap();
